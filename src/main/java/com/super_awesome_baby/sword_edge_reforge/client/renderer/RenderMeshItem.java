...
public class RenderMeshItem extends RenderItemBase {
    ...
    public RenderMeshItem(JsonElement jsonElement) {
        ...
        if (jsonObj.has("mesh1_main")) {
            meshLoc = jsonObj.get("mesh1_main").getAsString();
            resLoc = ResourceLocation.parse(meshLoc);
            this.mesh1_main = MeshAccessor.create(resLoc.getNamespace(), resLoc.getPath(), (loader) -> {
                return loader.loadSkinnedMesh(SkinnedMesh::new);
            });
        } else {
        ...
        if (jsonObj.has("mesh1_off")) {
            meshLoc = jsonObj.get("mesh1_off").getAsString();
            resLoc = ResourceLocation.parse(meshLoc);
            this.mesh1_off = MeshAccessor.create(resLoc.getNamespace(), resLoc.getPath(), (loader) -> {
                return loader.loadSkinnedMesh(SkinnedMesh::new);
            });
        } else {
            this.mesh1_off = null;
        ...
        if (jsonObj.has("mesh2_main")) {
            meshLoc = jsonObj.get("mesh2_main").getAsString();
            resLoc = ResourceLocation.parse(meshLoc);
            this.mesh2_main = MeshAccessor.create(resLoc.getNamespace(), resLoc.getPath(), (loader) -> {
                return loader.loadSkinnedMesh(SkinnedMesh::new);
            });
        } else {
        ...
        if (jsonObj.has("mesh2_off")) {
            meshLoc = jsonObj.get("mesh2_off").getAsString();
            resLoc = ResourceLocation.parse(meshLoc);
            this.mesh2_off = MeshAccessor.create(resLoc.getNamespace(), resLoc.getPath(), (loader) -> {
                return loader.loadSkinnedMesh(SkinnedMesh::new);
            });
        } else {
            this.mesh2_off = null;
    ...
    public void renderItemInHand(ItemStack stack, LivingEntityPatch<?> livingEntityPatch, InteractionHand hand,
        ...
        DynamicAnimation animation = Objects.requireNonNull(livingEntityPatch.getAnimator().getPlayerFor(null)).getAnimation().get();
        ...
        if (useMesh2) {
            renderMesh = (hand == InteractionHand.MAIN_HAND) ?
                    this.mesh2_main.get() : this.mesh2_off.get();
            texture = mesh2Texture;
            textureL = mesh2TextureL;
        } else {
            renderMesh = (hand == InteractionHand.MAIN_HAND) ?
                    this.mesh1_main.get() : this.mesh1_off.get();
            texture = mesh1Texture;
            textureL = mesh1TextureL;
        }

        if (renderMesh != null) {
            if (texture != null) {
                renderMesh.draw(poseStack, buffer, RenderType.entityTranslucent(texture), packedLight,
                        1.0F, 1.0F, 1.0F, 1.0F, OverlayTexture.NO_OVERLAY, armature, armature.getPoseMatrices());
            }

            if (textureL != null) {
                renderMesh.draw(poseStack, buffer, RenderType.entityTranslucentEmissive(textureL), packedLight,
                        1.0F, 1.0F, 1.0F, 0.9F, OverlayTexture.NO_OVERLAY, armature, armature.getPoseMatrices());
            }
        }
